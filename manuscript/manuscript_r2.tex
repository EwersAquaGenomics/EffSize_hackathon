...
\section{Abstract}
Inference of effective population size over time from genomic data is of paramount importance in a multitude of disciplines from conservation biology to epidemiology of infectious diseases. It provides insight about past evolutionary dynamics from present-day genomic data. Here, we present a new \texttt{R} package, called \texttt{multiNe}, that allows us to infer effective population size trajectories from genealogies under the coalescent framework and from allele frequencies directly. Our coalescent framework provides nonparametric Bayesian estimates of effective population size over time from genealogical data. In addition, we implement two alternative methods for inferring $N_{e}$ from genotypic data and allele frequencies. Our implementation will allow for the interaction of our package with other packages implemented in \texttt{R} to infer effective population size trajectories from sequence data (i.e, \texttt{coalescentmcmc} and \texttt{phangorn}).
...
%These Wright-Fisher estimators calculate $N_{e}$ over the past few generations, whereas coalescent estimators integrate $N_e$ over the past $N_e$ to 4 * $N_e$ generations.
